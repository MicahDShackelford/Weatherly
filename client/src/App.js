...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currentLocation : {zip: '37912', data: {}, forecast: []},
      locations : []
    }
  }
  ...
  fetchData () {
    fetch(`http://api.openweathermap.org/data/2.5/weather?zip=${this.state.currentLocation.zip},us&APPID=${API_KEY}`, {
      ...
      this.setState({
        currentLocation: {zip:this.state.currentLocation.zip, data: res, forecast: this.state.currentLocation.forecast}
      });
    });
    fetch(`http://api.openweathermap.org/data/2.5/forecast?zip=${this.state.currentLocation.zip},us&APPID=${API_KEY}`, {
      method: 'GET'
    }).then((res) => {
      return res.json();
    }).then((res) => {
      this.setState({
        currentLocation: {zip:this.state.currentLocation.zip, data: this.state.currentLocation.data, forecast: res}
      });
    })
  }
  ...
  render(){
    return (
      <div className="App">
        ...
        <AddLocation />
        <Forecast data={this.state.currentLocation} />
      </div>
    );
  }
}
